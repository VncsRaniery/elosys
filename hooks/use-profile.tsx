...
export function useProfile(initialProfile: Profile) {
  ...
  const updateSecondaryBg = (bgColor: string) => {
    ...
    if (mainElement) {
      // Como mainElement existe, não precisamos mais do `?.`
      const oldBgClasses = mainElement.classList.value
        .split(" ")
        .filter((cls) => cls.startsWith("bg-"));

      mainElement.classList.remove(...oldBgClasses);
      mainElement.classList.add(bgColor);
    }
  };
  ...
  useEffect(() => {
    const mainElement = document.querySelector("main");
    if (mainElement && profile.secondaryBg) {
      const oldBgClasses = mainElement.classList.value
        .split(" ")
        .filter((cls) => cls.startsWith("bg-"));

      mainElement.classList.remove(...oldBgClasses);
      mainElement.classList.add(profile.secondaryBg);
    }
  }, [profile.secondaryBg]);
